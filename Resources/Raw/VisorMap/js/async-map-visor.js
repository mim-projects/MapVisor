...
class Stage {
    ...
    constructor(htmlElement, debug = false, sizeControlEvents = "1.25rem") {
        this.#actors = []
        ...
        this.#eventsHelper = {
            onMousePress: false,
            sizeControlEvents: sizeControlEvents
        }
    ...
    removeForType(type) {
        this.#actors = this.#actors.filter(item => item.data.category !== type)
    }

    filter(param, value) {
        return this.#actors.filter(item => item.data[param] !== undefined && item.data[param] === value)
    }

    async addAsync(actor, render = true) {
        this.#actors.push(actor)
        await new Promise((resolve, reject) => {
            actor.image.onerror = (e) => reject(e)
            actor.image.onload = () => {
                actor.update()
    ...
    #events(htmlElement) {
        ...
        this.#context.canvas.addEventListener("mousedown", (e) => {
            ...
            if (this.#context.canvas.style.cursor !== "pointer") {
                ...
                const y = e.pageY - e.currentTarget.offsetTop - this.#context.canvas.getBoundingClientRect().y - window.scrollY
        ...
        const styleButton = {
            width: this.#eventsHelper.sizeControlEvents,
            height: this.#eventsHelper.sizeControlEvents,
            padding: "0.25rem",
            userSelect: "none",
            cursor: "pointer",
            display: "flex",
